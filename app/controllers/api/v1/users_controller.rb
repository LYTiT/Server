class Api::V1::UsersController < ApiBaseController
	...
	def get_aggregate_feed_activity
		...
		@activity = Kaminari.paginate_array(FeedActivity.where("id > ? AND feed_id IN (?) AND (NOW() - created_at) <= INTERVAL '1 DAY'", last_activity_id, user_feed_ids).includes(:user, :venue, :venue_comment, :feed).order("adjusted_sort_position DESC")).page(params[:page]).per(10)
	end
	#-------------------------------------------------->
