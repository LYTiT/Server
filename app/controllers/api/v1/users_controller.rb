class Api::V1::UsersController < ApiBaseController
	...
	def get_bounties
		...
		raw_bounties = Bounty.where("((user_id = ? AND validity = TRUE) OR ((id IN (#{subcribed_bounty_ids}) AND user_id != ?) AND (validity = TRUE AND num_responses > 0 AND (NOW() - created_at) <= INTERVAL '1 DAY'))", params[:user_id], params[:user_id]).includes(:venue).order('id DESC')
		@bounties = []
