class Api::V1::FeedsController < ApiBaseController
	...
	def search
		...
		@feeds = (Feed.where("LOWER(name) like (?)", params[:q].downcase+"%").inclueds(:feed_users).limit(10)+Feed.search(params[:q]).includes(:feed_users).limit(10)).uniq#Feed.search(params[:q]).includes(:feed_users).limit(15)
	end
