class Api::V1::FeedsController < ApiBaseController
	...
	def create
		feed = Feed.new(:name => params[:name].strip, :user_id => params[:user_id], :latest_viewed_time => Time.now, :feed_color => params[:feed_color], :open => params[:open], :description => params[:list_description])
		feed.save
	...
	def get_venues
		...
		@added_venue_activities = FeedVenue.where("feed_id = ?", params[:id]).includes(:activity, :venue, :user).order("name ASC").page(params[:page]).per(15)
	end
