class Api::V1::VenuesController < ApiBaseController
	...
	def get_surrounding_feed_for_user
		...
		@venues = Kaminari.paginate_array(Venue.within(Venue.meters_to_miles(meter_radius.to_i), :origin => [lat, long]).includes(:venue_comments).order('distance ASC limit 8').to_a).page(params[:page]).per(2)
	end
