class Api::V1::VenuesController < ApiBaseController
	...
	def post_comment
		...
		if params[:venue_id] != nil
			...
			venue = Venue.fetch_or_create(params[:name], params[:formatted_address], params[:city], params[:state], params[:country], 
				params[:postal_code], params[:phone_number], params[:latitude], params[:longitude], is_proposed_location)
		end

		user = User.find_by_authentication_token(params[:auth_token])
		
		if params[:media_type] == "video"
			image_url = params[:video_url_3].gsub(".mp4", ".jpg")
		else
		...
		post = {:comment => params[:comment], :media_type => params[:media_type], :media_dimensions => params[:media_dimensions], 
			...
			:video_url_2 => params[:video_url_2], :video_url_3 => params[:video_url_3], :created_at => Time.now, :reaction => params[:reaction], 
			:position_index => (Time.now+30.minutes).to_i}
	...
	def get_comments_feed
		...
		if Rails.cache.exist?(@view_cache_key) == true and (@venue.latest_posted_comment_time != nil and @venue.latest_posted_comment_time > Time.now - 5.hours)
			...
			render 'venue_comment_feed.json.jbuilder'
		else
			p "Rebuilding views"
	...
	def refresh_map_view_by_parts
		...
		if Rails.cache.exist?(@view_cache_key) == true
			...
			if params[:version] == "1.1.0"
				...
				render "map_view_refresh.json.jbuilder"
			end
			...
			@venues = Rails.cache.fetch(city_cache_key, :expires_in => 10.minutes) do
				...
				if page == 1
					venues = Venue.close_to(lat, long, 5000).select("id, name, address, city, country, latitude, longitude, color_rating, rating, popularity_rank, instagram_location_id, latest_posted_comment_time, venue_comment_details, event_details, trending_tags, categories").where("color_rating > -1.0").order("color_rating DESC").limit(num_page_entries).offset((page-1)*num_page_entries).to_a rescue "error"
				end
