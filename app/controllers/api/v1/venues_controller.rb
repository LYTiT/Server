...
#depricated
	...
	def refresh_map_view_by_parts
		...
		@view_cache_key = "#{city}/lyt_map/view/#{Time.now.to_date}/#{Time.now.hour}/#{time_key}/page_#{page}v1"
		...
		if Rails.cache.exist?(@view_cache_key) == true
			...
			@view_cache_key = "#{city}/lyt_map/view/#{Time.now.to_date}/#{Time.now.hour}/#{previous_time_key}/page_#{page}v1"
			if params[:version] == "1.1.0"
				render "refresh_map_view_by_parts_old.json.jbuilder"
			else
				render "refresh_map_view_by_parts.json.jbuilder"
			...
			@venues = Rails.cache.fetch(city_cache_key, :expires_in => 10.minutes) do
				...
				if page == 1
					venues = Venue.close_to(lat, long, 5000).select("id, name, address, city, country, latitude, longitude, color_rating, popularity_rank, instagram_location_id, venue_comment_details, event_details, trending_tags, categories").where("color_rating > -1.0").order("color_rating DESC").limit(num_page_entries).offset((page-1)*num_page_entries).to_a rescue "error"
				end

				if page == 1 and venues.length == 0
					venues = Venue.far_from(lat, long, 5000).select("id, name, address, city, country, latitude, longitude, color_rating, popularity_rank, instagram_location_id, venue_comment_details, event_details, trending_tags, categories").where("color_rating > -1.0").order("color_rating DESC").limit(num_page_entries).offset((page-1)*num_page_entries).to_a rescue "error"
				elsif page > 1
