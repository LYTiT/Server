...
=end		
	...
	def refresh_map_view
		...
		@venues = Rails.cache.fetch(cache_key, :expires_in => 5.minutes) do
			Venue.where("rating IS NOT NULL").to_a
		end
	...
	def refresh_map_view_by_parts
		...
		if page == 1
			...
			faraway_venues = Rails.cache.fetch(cache_key, :expires_in => 10.minutes) do
				Venue.far_from(center_point.first, center_point.last, 5000).where("rating IS NOT NULL AND latest_posted_comment_time > ?", (Time.now - LytitConstants.threshold_to_venue_be_shown_on_map.minutes)).order("id DESC").limit(num_page_entries).offset((page-2)*num_page_entries).to_a
