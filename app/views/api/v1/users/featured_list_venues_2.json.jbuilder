json.activity(@activities) do |activity|
  json.feed_id Feed.joins(:feed_venues, :feed_users).where("feed_venues.venue_id = ? AND feed_users.user_id = ?", venue.id, user.id).order("feed_users.interest_score DESC").first.try(:id)
