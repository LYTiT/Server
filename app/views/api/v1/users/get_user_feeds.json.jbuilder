json.array! @feeds do |feed|
	...
	json.venue_added feed.feed_venue.where("venue_id =?", @venue.try(:id)).nil?
	...
	json.has_added feed.feed_user.where("user_id = ?", @user.id).nil?
	json.list_description feed.description
	json.subscribed feed.feed_user.where("user_id = ?", @user.id).try(:is_subscribed)
