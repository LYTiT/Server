json.cache! @venues, expires_in: 10.minutes, key: @view_cache_key do |json|
  json.activity(@venues) do |venue|
    ...
    json.tag_1 MetaData.where("venue_id = ?", venue.id).order("relevance_score DESC").limit(1).offset(1).first.try(:meta)
    json.tag_2 MetaData.where("venue_id = ?", venue.id).order("relevance_score DESC").limit(1).offset(2).first.try(:meta)
    json.tag_3 MetaData.where("venue_id = ?", venue.id).order("relevance_score DESC").limit(1).offset(3).first.try(:meta)
    json.tag_4 MetaData.where("venue_id = ?", venue.id).order("relevance_score DESC").limit(1).offset(4).first.try(:meta)
    json.tag_5 MetaData.where("venue_id = ?", venue.id).order("relevance_score DESC").limit(1).offset(5).first.try(:meta)
