class VenueQuestionComment < ActiveRecord::Base
	...
	def self.new_comment(v_q_id, comment, v_id, u_id, comment_user_on_location)
		new_venue_question_message = VenueQuestionComment.create!(:venue_question_id => v_q_id, :user_id => u_id, :comment => comment)
		new_venue_question_message.venue_question.increment(:num_comments, 1)
		new_venue_question_message.new_question_message_notification(comment_user_on_location)
	end
	...
	def new_question_message_notification(comment_user_on_location)
		...
		for question_participant in question_participants
			if question_participant.id != self.user_id
				self.send_new_question_message_notification(question_participant, comment_user_on_location)
			end
		end
