...
#===============================================================================================
  ...
  def latest_rating
    ...
    new_r_up_vote_count = ((old_r_up_vote_count) * 2**((-(Time.now.utc - latest_posted_comment_time)/60.0) / (vote_half_life))).round(4)
    
    y = (1.0 / (1 + rating_loss)).round(4)
...
def Venue.live_recommendation_for(user, lat=40.741140, long=-73.981917)
  ...
  if top_user_interests.count > 0
    ...
    if results.length < 30
      if results.length == 0
        Venue.in_bounds(search_box).order("popularity_rank DESC").where("color_rating > -1.0").limit(30-results.length)
      else
        results += Venue.in_bounds(search_box).order("popularity_rank DESC").where("color_rating > -1.0").limit(30-results.length)
      end
    end
  else
