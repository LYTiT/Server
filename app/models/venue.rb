      ...
      if super_content_count > 0 && (page_offset == 0 || page_number*page_count <= page_offset)
        ...
        if (self.last_instagram_pull_time == nil or self.last_twitter_pull_time == nil) or ((Time.now - api_ping_timeout) > self.last_instagram_pull_time or (Time.now - api_ping_timeout) > self.last_twitter_pull_time)
          ...
          if new_social_media_num_pages > 0
            ...
            vcs = self.venue_comments.where("((adjusted_sort_position > ? AND entry_type != ?) OR adjusted_sort_position > ?) AND adjusted_sort_position < ? AND id <= ?", (Time.now-5.hours).to_i, "lytit_post", (Time.now-24.hours).to_i, current_position, self.venue_comment_id).limit(page_count).offset(((page_number-self.page_offset)-1)*page_count).order("adjusted_sort_position DESC")
            if vcs.count > 0
              vcs.to_a
            else
              nil
