...
def recommendation_reason_for(user)
  ...
  if user_list != nil
    ...
    top_user_interests = user.top_interests(6)
    venue_meta = self.categories.values+self.descriptives.keys
    ...
    if interest_match == true
      for interest in top_user_interests
        if Venue.interest_search(interest).where("id = ?", self.id).first != nil
          sources = user.interests["descriptives"][interest].keys || user.interests["venue_categories"][interest].keys
          if sources.contains? "searched_venue_ids"
