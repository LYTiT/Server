class Venue < ActiveRecord::Base
  ...
  def self.geo_spotlyt(radius, lat, long, start_t, end_t)
    ...
    images = VenueComment.where("media_type = 'image' AND (created_at + local_time_offset) < ? AND (created_at + local_time_offset) >= ? AND venue_id in (#{venue_ids})", end_t, start_t)
    videos = VenueComment.where("media_type = 'video' AND (created_at + local_time_offset) < ? AND (created_at + local_time_offset) >= ? AND venue_id in (#{venue_ids})", end_t, start_t)
