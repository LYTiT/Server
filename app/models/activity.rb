...
#featured_list_venue
	...
	def Activity.create_featured_list_venue_activity(featured_venue_entry, content, user_id, feed_id, feed_name, feed_color)
		if feed_id == nil
			feed = Feed.joins(:feed_venues, :feed_users).where("feed_venues.venue_id = ? AND feed_users.user_id = ?", conten.venue_id, user_id).order("feed_users.interest_score DESC").first
			feed_id = feed.id
			feed_name = feed.name
		...
		if content.class.name == "VenueComment"
			if Activity.where("feed_id = ? AND activity_type = ? AND venue_comment_id = ?", feed_id, "featured_list_venue", content.id).any? == false
				new_activity = Activity.create!(:feed_id => feed_id, :feed_name => feed_name,
					:feed_color => feed_color, :activity_type => "featured_list_venue",
					:tag_1 => MetaData.where("venue_id = ?", activity["id"]).order("relevance_score DESC").limit(1).offset(1).first.meta, 
					:tag_2 => MetaData.where("venue_id = ?", activity["id"]).order("relevance_score DESC").limit(1).offset(2).first.meta, 
					:tag_3 => MetaData.where("venue_id = ?", activity["id"]).order("relevance_score DESC").limit(1).offset(3).first.meta,
					:tag_4 => MetaData.where("venue_id = ?", activity["id"]).order("relevance_score DESC").limit(1).offset(4).first.meta, 
					:tag_5 => MetaData.where("venue_id = ?", activity["id"]).order("relevance_score DESC").limit(1).offset(5).first.meta, 
