class User < ActiveRecord::Base
  ...
  def aggregate_list_feed
    ...
    Activity.where("id IN (#{activity_ids}) AND (NOW() - created_at) <= INTERVAL '1 DAY' AND adjusted_sort_position IS NOT NULL").includes(:user, :venue, :venue_comment).order("adjusted_sort_position DESC")
  end
