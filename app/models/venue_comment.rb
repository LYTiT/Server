class VenueComment < ActiveRecord::Base
	...
	def self.get_comments_for_cluster(venue_ids)
		VenueComment.where("venue_id IN (?) AND (NOW() - created_at) <= INTERVAL '1 DAY' AND (user_id IS NOT NULL AND content_origin = ?)", venue_ids, "lytit").includes(:venue, :bounty, bounty: :bounty_subscribers).order("time_wrapper desc")
	end
